...
读取 _安装状态 tab，确认这是全新安装还是升级已有安装
        |
        v
创建/升级 Google Sheet 结构（十个用户数据 tab + 一个内部 _安装状态 tab）
        |
        v
...
- 如果是**已有 Sheet**：先尝试读取其中的 `_安装状态` tab（第 2 步 schema 里定义的内部安装状态
  ...
  - 无论哪种情况，读取现有 tab 结构与下面的 schema 对比，只做**增量**修改（缺少的 tab/字段才
    ...
    数据。
- 如果是**全新安装**：新建一个 Sheet，按下面的 schema 创建全部十个用户数据 tab，以及第 2 步定义
  的内部 `_安装状态` tab。
...
（`schema_version: 4`）逐字一致，如果你能直接读取该文件，以文件内容为准；这里的表格是供你在无法
读取仓库文件时直接使用的完整副本：
...
| `入场计划` | `计划ID*`, `代码*`, `版本*`, `上一版本ID`, `状态*`, `数据时间*`, `长期估值锚点`, `计划新增仓位说明`, `区间1价格`, `区间1占比`, `区间2价格`, `区间2占比`, `区间3价格`, `区间3占比`, `技术确认条件`, `不追高条件`, `失效条件`, `下一验证证据`, `计划复核触发`, `依据来源`, `备注` |
| `报告摘要` | `报告ID*`, `生成ID*`, `报告类型*`, `报告日期*`, `周期开始`, `周期结束`, `一句话结论*`, `核心状态*`, `Review状态`, `当前持仓数`, `高优先级观察数`, `生成时间*`, `备注` |
| `报告持仓` | `报告ID*`, `生成ID*`, `报告类型*`, `报告日期*`, `资产类型*`, `代码*`, `名称*`, `数量`, `Thesis状态`, `IOS状态`, `关键变化`, `长期逻辑或估值`, `下一验证点`, `参考买入价/区间`, `当前价格`, `价格位置`, `优先级`, `排序`, `入场计划ID`, `执行状态`, `当前区间`, `需要复核`, `入场计划估值锚点` |
...
- 每个**用户数据** tab（前十个，含新增的 `账户状态`/`投资信托明细`/`入场计划`）第一行必须是表头（字段名逐字
...
- `入场计划` tab（含 `计划新增仓位说明`——区间占比的说明性、非可执行分母，见 schema 文件该字段的
  注释）和 `报告持仓` 末尾的 `入场计划ID`/`执行状态`/`当前区间`/`需要复核`/`入场计划估值锚点` 五个
  字段是 schema_version 4 新增的**可选**内容（第 298 号 issue）：这一步只创建空表头（`入场计划`）
  和新增五列（`报告持仓`），不需要回填任何历史行，也不产出任何示例计划数据——`入场计划` 的实际内容
  只由 Weekly Deep Dive 创建/更新，`报告持仓` 那五个字段的实际取值只由 Daily/Weekly Task 在生成新
  报告行时写入；安装/升级步骤本身绝不主动询问用户、也绝不编造任何估值锚点、区间价格或执行状态。
  tab/字段完全不存在时，Dashboard 和 Daily/Weekly Task 都会退回到这个功能存在之前的展示方式。
- 增量升级时，只追加缺失的 tab/字段（例如在已有 tab 末尾新增列，或新增整个 `账户状态`/
  `投资信托明细`/`入场计划` tab，或在 `报告待办`/`报告持仓` 末尾新增列），不得重新排序或覆盖已有
  列——用户已经填写的单元格必须原样保留。任何会导致现有行无法按原样读取的改动（重命名字段、改变
  字段含义）都不是这里说的"增量"，必须走独立的、有备份和用户批准的迁移步骤，绝不能夹在普通安装/
  升级流程里悄悄执行。升级完成后把 `_安装状态.schema_version` 更新为当前最新版本 `4`——无论升级前
  是 schema_version 1、2 还是 3，增量升级后都收敛到同一个最新版本，缺失的列/tab 一次性补齐。
...
4. **Weekly 现金/资金状态（仅 Weekly Task）**：这项必要输入只有一个权威来源，由 Sheet 当前的
   ...
   - **本次运行会创建/保持 `账户状态` tab**（第 2 步已经创建了它——这是现在起每一次全新安装或升级
     ...
     `_安装状态.schema_version` 是当前最新版本 `4`）：**不要**再问"简短描述"这类自由文本问题。把
...
- [ ] Sheet 中十个用户数据 tab（含 schema_version 2 新增的 `账户状态`/`投资信托明细`、
      schema_version 4 新增的 `入场计划`）都存在，表头与上面的表格逐字匹配（含 `报告待办` 末尾
      schema_version 3 新增的 `跟进类型` 列，以及 `报告持仓` 末尾 schema_version 4 新增的
      `入场计划ID`/`执行状态`/`当前区间`/`需要复核`/`入场计划估值锚点` 五列，`入场计划` 表头末尾
      新增的 `计划新增仓位说明` 列）；`_安装状态` tab 也存在，且只有一行数据。
...
- [ ] `账户状态` tab 已创建（空表头，安装过程没有替用户填入任何编造的具体数值）。**如果**本次
      ...
      全新安装或增量升级都是如此，最终记录的 `_安装状态.schema_version` 是当前最新版本 `4`）：
...
- [ ] `入场计划` tab 已创建（空表头，含 `计划新增仓位说明` 列），`报告持仓` 表头末尾已有
      `入场计划ID`/`执行状态`/`当前区间`/`需要复核`/`入场计划估值锚点` 五列（schema_version 4，第
      298 号 issue），本次安装/升级步骤没有替这六处填入任何示例计划或估值数据——`入场计划` 的实际
      内容只由 Weekly Deep Dive 创建/更新，`报告持仓` 那五列只由 Daily/Weekly Task 在生成新报告行
      时写入。
...
- 不在安装/升级步骤里往 `报告待办.跟进类型`（schema_version 3，第 267 号 issue）填入任何值——这一
  ...
  替已有的历史 `报告待办` 行补写这一列。
- 不在安装/升级步骤里往 `入场计划`（schema_version 4，第 298 号 issue）写入任何示例行，也不往
  `报告持仓` 新增的 `入场计划ID`/`执行状态`/`当前区间`/`需要复核`/`入场计划估值锚点` 五列填入任何
  值——`入场计划` 的实际内容只由 Weekly Deep Dive 创建/更新，那五列只由 Daily/Weekly Task 生成报告
  时写入，安装程序本身不产出计划或报告数据。
